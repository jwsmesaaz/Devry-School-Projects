...
namespace CIS407Project
{
    public partial class WebForm3 : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (blnAllWhite == true)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    setWarningMessage("Start Date is not a valid format");
                    //Tells the boolean that not everything is alright
                    blnAllWhite = false;
                }
                try
                ...
                {
                    //This sets the warning message so you are able to give a list of items that need to be fixed
                    setWarningMessage("End Date is not a valid format");
                    //Tells the boolean that not everything is alright
                    blnAllWhite = false;
                }
                if (blnAllWhite == true)
            ...
            try
            {
                ...
                {
                    //Double dblPayRate = Double.Parse(txtEndDate.Text);
                    //Sets Background color to White                
            ...
            {
                //Adds to the warning message
                setWarningMessage("Pay Rate mustr contain only numbers");
                //Tells the boolean that not everything is alright
                blnAllWhite = false;
                //Puts the curser in the box if this is blank
                txtPayRate.Focus();
        ...
        private void setWarningMessage(string strWarningMessage)
        {
            ...
            if (Session["warningMessage"] == null || Session["warningMessage"] == "")
            {
